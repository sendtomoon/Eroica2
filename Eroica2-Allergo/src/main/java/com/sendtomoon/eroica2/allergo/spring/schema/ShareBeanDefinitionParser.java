...
public class ShareBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		try {
			...
			throw new BeanDefinitionValidationException(
					"PizzaBean register  error by config:" + configURLString + ",cause:" + ex.getMessage(), ex);
		}
		// ---------------------------------
		return null;
	}
	...
	protected BeanDefinition loadBeanDefinition(String type, AllergoURL allergoURL, URLUtils configURL)
		...
		try {
			ExtensionLoader<PizzaShareBean> loader = ExtensionLoader.getExtensionLoader(PizzaShareBean.class);
			PizzaShareBean factory = loader.getExtension(type);
